...
Moves to the specified angular position. This button cannot be pressed if alternative mode is selected (due to manual stepping), or if the selected motor is either already moving or queued up to move
If MOVE is called when the specified motor is at the HOME position, then the motor will not move the motor and will instead zero its position.
If the motor hits the HOME position during rotation, then the motor will stop and wait a specified amount of time before rotating a specified amount of steps in the opposite direction.

	CALIBRATE

Rotates the wavemount in the same direction each time until the pin hits the HOME position, then waits a specified amount of time before rotates a specified amount of steps in the opposite direction.
If CALIBRATE is called when the specified motor is at the HOME position, then the motor will not move the motor and will instead zero its position.
...
Moves the motor to the specified position. Maximum steps is +/- 358400 full steps.
...
Sets the current position to zero.
If alternative mode is enabled by alt.toggle, calib.w triggers waveplate calibration.
If the specified motor is currently queued up to move, then calib.w will do nothing.

	speed.w [motor number] [speed]

Sets the motor speed in steps/second. The minimum speed is 15.38 steps/second, with a maximum of 10k steps/second. The default speed for all motors is set at 500 steps/second.
...
2) While motors are moving, don't toggle motors on other boards on/off (e.g. if motor #3 is moving, don't switch motor #7 on/off) or change their holding voltage. However, changing moving voltage is ok.
	2.1) This has been fixed in v1.2.0 by preventing user from executing these commands.

	Alternative mode, and manual moving:

Alternative mode can be toggled by either entering "alt.toggle" into the serial interface, or by pressing both the switches simultaneously without any boxes selected on the screen. This enables 1) manual stepping, and 2) motors with a "HOME" sensor to cross "HOME". Note that when entering alternative mode, the step displays will reset to the last moved position, abandoning any "unmoved" steps.
In manual stepping, the encoders and switches directly move the motors without the need to press "MOVE". With manual stepping, motors move at a maximum of 333 steps/second. "move.w" is unaffected.

	"HOME" Sensor

If a motor crosses the "HOME" position, it will stop for a user-specified time, then move in the opposite direction for a user-specified number of steps.
If move or calibrate commands are called while a motor is at the HOME position, it will not move and instead zero its position.
...
However, the stepping delay has an error of about 10us due to the time it takes to process instructions within the timer interrupt. At the highest speed setting (10k steps/second) this creates about 9% error, but at the base speed (500 steps/second) we see 0.5% error.
