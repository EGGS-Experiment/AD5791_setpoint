...
2) While motors are moving, don't turn other motors on/off (e.g. if motor #3 is moving, don't switch motor #7 on/off). However, changing voltage is ok.
	...
	Alternative mode, waveplate calibration, and manual moving:

Alternative mode can be toggled by either entering "alt.toggle" into the serial interface, or by pressing both the switches simultaneously without any boxes selected on the screen. This enables 1) manual stepping, and 2) alternate calibration in motors with a "HOME" sensor. Alternately, alternative mode can be toggled by pressing both switches together when nothing is selected on screen.
...
In alternate calibration, "calib.w" and the "CALIBRATE" button trigger waveplate calibration, which moves the motor until the attached sensor (the motor's DB9 Pin 9) reads true. For waveplate calibration to be enabled for a motor, motors need to be declared as having a sensor in the code (lines 171-182).
